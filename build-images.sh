...
# Reuse existing nodebuilder-keila container, to speed up builds
if ! buildah containers --format "{{.ContainerName}}" | grep -q nodebuilder-keila; then
    echo "Pulling NodeJS runtime..."
    buildah from --name nodebuilder-keila -v "${PWD}:/usr/src:Z" docker.io/library/node:lts
fi
...
buildah config --entrypoint=/ \
    ...
    --label="org.nethserver.images=docker.io/postgres:15.5-alpine3.19 docker.io/pentacent/keila:${keila_version}" \
    "${container}"
# Commit the image
